...
const quantmn = 'https://ipfs.io/ipfs/QmZUpW1Q41BzdUMWwaH1CEXAoa8hfdRoREZ58kHiTzPe5n'
...
async function setupMarket (marketplaceAddress, nftAddress) {
  ...
  console.log({ marketplaceAddress, nftAddress })

  const price = hre.ethers.utils.parseEther('0.01')
  const listingFee = await marketplaceContract.getListingFee()

  const quantMintTx = await nftContract.mintToken(quantmn)
  const quantTokenId = await getMintedTokenId(quantMintTx)
